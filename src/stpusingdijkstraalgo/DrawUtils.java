...
public class DrawUtils {
    ...
    public static boolean isOnEdge(MouseEvent e, Edge edge) {
        ...
        int dist = distToSegment(e.getPoint(),
                edge.getStartVertex().getCoord(),
                edge.getTargetVertex().getCoord());
        if (dist < 6) {
            return true;
        }
        return false;
    ...
    public void drawWeight(Edge edge) {
        Point from = edge.getStartVertex().getCoord();
        Point to = edge.getTargetVertex().getCoord();
        int x = (from.x + to.x) / 2;
        int y = (from.y + to.y) / 2;

        int rad = radius / 2;
        g.fillOval(x - rad, y - rad, 2 * rad, 2 * rad);
        drawWeightText(String.valueOf(edge.getTime()), x, y);
    }

    public void drawPath(java.util.List<Vertex> path) {
    ...
    public void drawDeadEdge(Edge edge) {
        g.setColor(parseColor("#8B0000"));
        drawBaseEdge(edge);
        drawWeight(edge);
    }
...
//        drawArrowHead(g, to, from, parseColor("#003500"));
        g.setStroke(new BasicStroke(3));
        g.drawLine(from.x, from.y, to.x, to.y);
    }

    public void drawHalo(Vertex node) {
    ...
    public void drawSourceNode(Vertex node) {
        g.setColor(parseColor("#00BCD4"));
        g.fillOval(node.getX() - radius, node.getY() - radius, 2 * radius, 2 * radius);
        ...
        radius += 5;
        g.setColor(parseColor("#00BCD4"));
        drawCentreText(node.getName(), node.getX(), node.getY());
    }

    public void drawDestinationNode(Vertex node) {
    ...
    public void drawNode(Vertex node) {
